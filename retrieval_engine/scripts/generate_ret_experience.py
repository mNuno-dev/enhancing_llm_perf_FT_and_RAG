...
----- (required args) -----
python3 scripts/generate_ret_experience.py 3 /path/to/retrieval_evaluation/results "['BAAI/bge-m3','PORTULAN/serafim-100m-portuguese-pt-sentence-encoder-ir']" --batch_size 32
...
def main():
    ...
    # Retrieve collection names for model_name
    ...
    # Retrieving all the avaiable collections for a certain model
    for model_name in args.model_list:
        for col_name in available_collections:
            if aliases[model_name] in col_name.lower(): # .lower() because weaviate capitalizes the first character of a collection name
    ...
    for model_name in args.model_list:
        # Generate question embeddings for model_name
        q_embeddings = generate_embeddings_w_model(model_name=model_name, text=q_list, batch_size=args.batch_size)
