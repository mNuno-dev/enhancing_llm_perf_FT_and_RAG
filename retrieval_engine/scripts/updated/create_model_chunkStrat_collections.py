...
----- (required args) -----
python3 scripts/NEW/create_model_chunkStrat_collections.py hnsw "['BAAI/bge-m3','PORTULAN/serafim-100m-portuguese-pt-sentence-encoder-ir']"

---- (with optional args) ----
python3 scripts/NEW/create_model_chunkStrat_collections.py hnsw "['BAAI/bge-m3','PORTULAN/serafim-100m-portuguese-pt-sentence-encoder-ir']" --token [HF_TOKEN] --batch_size 32
"""
import re
import argparse
...
aux_fixed_size = [("fixedSize200",200)]
aux_fixed_size_overlap = [
    ("overlap200and02",[200,0.2])
]
...
def main():
    ...
    for model_name in args.model_list:
        ...
        for parameters in params_set_dict["fixed_size"]:
            ...
            embedded_chunks = embeddings.get_PTwikiRAG_chunkingStrat_embeddings(
        ...
        # Creating LXSentenceSplitter Collection
        LX_embedded_chunks = embeddings.get_PT_wikiRAG_LXsentenceSplit_embeddings(
        ...
        LXSentenceSplitter_collection_name = aliases[model_name] + "LXSentenceSplit"
        aux_list.append([LXSentenceSplitter_collection_name, LX_embedded_chunks])

        # Creating LXSentenceSplitter Collection
        DPC_embedded_chunks = embeddings.get_PTwikiRAG_DPCchunking_embeddings(
        ...
        DPC_chunks_collection_name = aliases[model_name] + "DPC_chunks"
        aux_list.append([DPC_chunks_collection_name, DPC_embedded_chunks])
