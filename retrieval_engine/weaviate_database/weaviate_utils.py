...
def query_with_external_embedding_call(
...
def query_with_own_embedding(
    collection_name: str,
    query_embedding: list,
    top_k: int = 3,
) -> List[Tuple[Dict[str, Any], Optional[float]]]:

    return _query_near_vector(collection_name, list(map(float, query_embedding)), top_k=top_k)
