...
class ResponseRelevancePrompt_PT(
    ...
    instruction = """Gera uma pergunta para a resposta dada e identifica se a resposta é evasiva. Dá o valor 1 para evasiva se a resposta for evasiva, e 0 para comprometida se a resposta for comprometida. Uma resposta evasiva é aquela que é vaga, ambígua ou que evita responder diretamente. Por exemplo, "Não sei" ou "Não tenho a certeza" são respostas evasivas."""
    ...
    examples = [
        (
            ResponseRelevanceInput(
                ...
                response="""Albert Einstein nasceu na Alemanha.""",
            ),
            ResponseRelevanceOutput(
                # question="Where was Albert Einstein born?",
                question="Onde nasceu Albert Einstein?",
                noncommittal=0,
            ),
        ),
        (
            ResponseRelevanceInput(
                ...
                response="""Não sei qual foi a característica inovadora dos smartphones inventada em 2023, pois não tenho informação para além de 2022.""",
            ),
            ResponseRelevanceOutput(
                # question="What was the groundbreaking feature of the smartphone invented in 2023?",
                question="Qual foi a característica inovadora dos smartphones inventada em 2023?",
                noncommittal=1,
            ),
        ),
    ]
...
@dataclass
class ResponseRelevancy_PT(MetricWithLLM, MetricWithEmbeddings, SingleTurnMetric):
    """
    ...
    question_generation: PydanticPrompt = ResponseRelevancePrompt_PT()
    strictness: int = 3
...
class AnswerRelevancy_PT(ResponseRelevancy_PT):
    async def _ascore(self, row: t.Dict, callbacks: Callbacks) -> float:
        return await super()._ascore(row, callbacks)


answer_relevancy_PT = AnswerRelevancy_PT()
